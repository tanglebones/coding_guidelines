...
## Content hashing: dedup and integrity, together

- **Compute a content hash of the file — client-side before/during upload, and server-side once the bytes are actually received — and use it for two purposes at once: deduplication (skip storing a file whose content already exists) and integrity verification (confirm what arrived is actually what was sent).**
- **Never trust a client-supplied hash on its own.** Same "never trust client input" discipline as everywhere else — always recompute the hash server-side from the bytes actually received, and reject/re-request if it doesn't match what the client claimed. A client-supplied hash is a claim to verify, not a fact to record.
- **Hash choice**: either match your storage backend's own content-addressing scheme (many object stores expose or accept an MD5/SHA-1-based checksum on write, so the storage layer's own integrity check and your dedup key can be the same value), or use a fast non-cryptographic hash (xxHash) purely for a cheap client-side pre-check, still followed by the storage backend's own verification once uploaded. This isn't a security-relevant hash choice the way the challenge-response protocol in `systems/login.md` needs bcrypt/SHA-512-class guarantees against a malicious actor — the threat model here is accidental duplication and transport corruption, not tamper-resistance, so MD5/SHA-1/xxHash are all fine; don't reach for a slow cryptographic hash out of habit when the goal is dedup and corruption detection, not tamper-proofing.
- **This is a real corruption check, not just a dedup convenience.** TCP's own checksum is weak and can occasionally miss real bit-level corruption; TLS's AEAD generally catches this on the wire, but re-verifying the hash once the file has actually landed in storage still closes gaps neither of those cover — a storage-layer bit flip, a bug in an intermediate proxy, a corrupted multipart-upload reassembly. Recomputing and checking the hash after the file is fully received and stored is cheap insurance regardless of how well-protected the transport itself is.
- **Dedup mechanics**: key stored objects by content hash (content-addressed storage), and have the upload-confirmation step check whether an object with this hash already exists before writing a new one — if it does, just create a new metadata row (in the same table described under "Storage lifecycle" below) pointing at the existing object instead of storing the bytes twice. This means deleting one upload's metadata row must never delete the underlying blob outright — check for other rows still referencing the same hash first (a reference count, or a query against the metadata table) before actually purging the object.
