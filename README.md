...
- Error handling has two accepted styles depending on the crate's needs — pick one deliberately:
  - `anyhow::Result<T>` + `bail!()`/`Context` for application-level code where callers don't need to match on error variants.
  - A hand-rolled error enum with manual `Display`, or a project-wide alias like `Rt<T> = Result<T, Box<dyn Error>>`, when you want a closed, matchable error set.
- `rustfmt`/`cargo fmt --check` and `cargo clippy --all-targets` (often `-D warnings`) enforced in CI; 2-space indent via `rustfmt.toml` is the deliberate house style, not default rustfmt.
...
- Set `MSYS2_ARG_CONV_EXCL="*"` for any script that must also run correctly under Windows/msys2.
...
- Encapsulate lifecycle-sensitive browser APIs in custom hooks (e.g. a hook wrapping `requestAnimationFrame` that also handles `cancelAnimationFrame` cleanup).
